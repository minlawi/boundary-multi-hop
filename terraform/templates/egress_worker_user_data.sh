#!/bin/bash
# Egress Worker User Data Script
...
# Target OS: Ubuntu
#
# Usage (manual):   sudo ./egress_worker_user_data.sh <intermediary_worker_private_ip>
...
log "Starting Boundary egress worker setup..."

# Configuration
# When rendered by Terraform templatefile(), the line below contains the actual IP.
# When running the raw template manually, it will be empty — pass the IP as $1 instead.
INTERMEDIARY_WORKER_PRIVATE_IP="${intermediary_worker_private_ip}"

# CLI argument overrides (for manual use)
if [ $# -gt 0 ] && [ -n "$1" ]; then
    INTERMEDIARY_WORKER_PRIVATE_IP="$1"
fi

if [ -z "$INTERMEDIARY_WORKER_PRIVATE_IP" ]; then
    log "ERROR: Intermediary worker private IP is required."
    log "Usage: sudo $0 <intermediary_worker_private_ip>"
    exit 1
fi
log "Intermediary worker IP (upstream): $INTERMEDIARY_WORKER_PRIVATE_IP"

# -----------------------------------------------
# Step 1: Get own private IP from instance metadata (IMDSv2)
# -----------------------------------------------
...
EGRESS_WORKER_PRIVATE_IP=$(curl -s --retry 5 --retry-delay 2 -H "X-aws-ec2-metadata-token: $IMDS_TOKEN" http://169.254.169.254/latest/meta-data/local-ipv4)
log "Egress worker private IP: $EGRESS_WORKER_PRIVATE_IP"
...
worker {
  initial_upstreams = ["$INTERMEDIARY_WORKER_PRIVATE_IP:9202"]
  public_addr = "$EGRESS_WORKER_PRIVATE_IP"
...
[Unit]
Description=Boundary Egress Worker
...
if systemctl is-active --quiet boundary-worker.service; then
    log "SUCCESS: Boundary egress worker service is running!"
else
    log "WARNING: Boundary egress worker service may not be running properly"
    systemctl status boundary-worker.service || true
fi

log "================================================"
log "BOUNDARY EGRESS WORKER SETUP COMPLETE!"
log "================================================"
